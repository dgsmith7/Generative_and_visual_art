...
//  sea: alph 45
  top = color (0, 230, 190, 45);

  middle = color(12, 210, 234, 45);

  bottom = color (90, 175, 225, 45);
...
// noStroke();
  ...
  for (float i = (numberOfLayers - 2); i > 0; i --) {
    ...
    if (i < 6) {fill(lerpColor(middle, top, 1 - (i / (numberOfLayers / 2))));}
    if (i == 6) {fill(middle);} 
    if (i > 6)  {println("yes"); fill(lerpColor(bottom, middle, 1 - ((i - 6) / (numberOfLayers / 2))));}
    
//    fill(lerpColor(middle, bottom, 1 - (i / numberOfLayers)));
