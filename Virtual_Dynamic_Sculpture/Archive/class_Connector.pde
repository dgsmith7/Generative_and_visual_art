class connector { 
 ...
 noStroke ();
    ...
    if ((x1 == x2) && (y1 == y2)) { // alligned on z axis
      ...
      box (nodeSize-shrinkFactor, nodeSize-shrinkFactor, -connectorLength+shrinkFactor);
    ...
    if ((x1 == x2) && (z1 == z2)) { // alligned on y axis
      ...
      box (nodeSize-shrinkFactor, connectorLength-shrinkFactor, nodeSize-shrinkFactor);
      translate(-x1, (-1*(y1+(.5*nodeSize)+(.5*connectorLength))), -z1);
    }
    if ((y1 == y2) && (z1 == z2)) { // alligned on x axis
      translate ((x1+(.5*nodeSize)+(.5*connectorLength)), y1, z1);
      box (connectorLength-shrinkFactor, nodeSize-shrinkFactor, nodeSize-shrinkFactor);
